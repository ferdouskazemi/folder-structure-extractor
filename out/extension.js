...
function buildTree(dirPath, options, currentDepth = 0) {
    ...
    for (const entry of sorted) {
        ...
        if (!isDir && !options.showFiles)
            continue;
        const node = { name: entry.name, isDir };
        if (isDir) {
...
function renderIcons(nodes, prefix = "") {
    let out = "";
    nodes.forEach((node, i) => {
        ...
        if (node.children?.length)
            out += renderIcons(node.children, prefix + childPfx);
    });
    return out;
}
function renderPlain(nodes, prefix = "") {
    let out = "";
    nodes.forEach((node, i) => {
        const last = i === nodes.length - 1;
        const branch = last ? "┗ " : "┣ ";
        ...
        if (node.children?.length)
            out += renderPlain(node.children, prefix + childPfx);
    });
    return out;
}
function buildWebview(panel, folderName, withIcons, noIcons) {
...
async function generate(uri, depth) {
    let folderPath;
    if (uri) {
    ...
    const showFiles = config.get("showFiles") ?? true;
    const options = { maxDepth: depth, ignore, showFiles };
    const nodes = buildTree(folderPath, options);
    if (!nodes.length) {
        const rawEntries = (() => { try {
    ...
    const panel = vscode.window.createWebviewPanel("folderStructure", `📁 ${folderName} — ${depthLabel}`, vscode.ViewColumn.Beside, { enableScripts: true });
    buildWebview(panel, folderName, renderIcons(nodes), renderPlain(nodes));
}
function activate(context) {
    context.subscriptions.push(vscode.commands.registerCommand("folderStructure.generateLevel1", (uri) => generate(uri, 1)), vscode.commands.registerCommand("folderStructure.generateLevel2", (uri) => generate(uri, 2)), vscode.commands.registerCommand("folderStructure.generateLevel3", (uri) => generate(uri, 3)), vscode.commands.registerCommand("folderStructure.generateCustom", async (uri) => {
        ...
        if (input)
            await generate(uri, parseInt(input));
    }));
}
function deactivate() { }
